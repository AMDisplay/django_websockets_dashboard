...
class DashboardConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self): # Вызывается когда инициируется связь с вебсокетом
    ...
    async def disconnect(self, close_code): # Вызывается когда соединение закрыто
        print(f'connection closed with code: {close_code}')
        await self.channel_layer.group_discard(
            self.room_group_name, self.channel_name
        )

    async def receive(self, text_data): # Получает данные из вебсокета
